...
namespace MIDIKeyboard.Miscellaneous
{
    ...
    public class Settings
    {
        ...
        public static bool Read()
        {
            ...
            foreach (string row in file) {
                if (row.Split(':').Length > 1) {
                    ...
                    PropertyInfo[] valueProperties = data.GetType().GetProperties(); //get data variables 
        ...
        public static void Save()
        {
            ...
            PropertyInfo[] valueProperties = data.GetType().GetProperties(); //get data values
            //PropertyInfo[] defaultValueProperties = defaultData.GetType().GetProperties(); //get default data values
            for (int i = 0; i < valueProperties.Length; i++) {
                //if (data.save_all || defaultValueProperties[i].GetValue(defaultData, null).ToString() != valueProperties[i].GetValue(data, null).ToString()) //check if data value differs from default data value
                    createText.Add(valueProperties[i].Name + ':' + valueProperties[i].GetValue(data, null)); //saves data value to string list
            }
